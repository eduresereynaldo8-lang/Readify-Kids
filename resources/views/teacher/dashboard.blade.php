...
<div class="ra-dashboard-grid">
    ...
    <section class="rk-card rk-attention">
        ...
        <div class="rk-table-scroll" tabindex="0" role="region" aria-label="Students needing support"><table class="rk-table"><thead><tr><th>Student</th><th>Class / Level</th><th>Average Final Score</th><th>Status</th></tr></thead><tbody>
            ...
            <tr><td><a href="{{ route('teacher.students.show', $learner->id) }}"><strong>{{ $learner->firstname }} {{ $learner->lastname }}</strong></a></td><td>{{ $learner->section ?: '—' }} · Level {{ $learner->current_level }}</td><td>{{ number_format($learner->activity_results_avg_score, 2) }}%</td><td><span class="rk-pill pending">{{ $learner->reading_status['label'] }}</span></td></tr>
...
<section class="rk-card" id="classroom-progress">
    ...
    @forelse($students as $learner)
        @php $average = $learner->activity_results_avg_score; $status = $learner->reading_status['label']; @endphp
